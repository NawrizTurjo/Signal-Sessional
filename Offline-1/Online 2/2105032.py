...
def main():
    ...
    INF_CURR = len(price_list)
    input_signal = DiscreteSignal(INF_CURR)
    for i in range(len(price_list)):
        input_signal.set_value_at_time(i,price_list[i])

    impulse_response = DiscreteSignal(INF_CURR)
    alphas = []
    curr_alpha=1
    for i in range(n):
        impulse_response.set_value_at_time(i,alpha*(curr_alpha))
        curr_alpha = curr_alpha*(1-alpha)
        # print(curr_alpha)
        # alphas.append(alpha*((1-alpha)**i))
    ...
    exsm = []
    exsm = output_signal.values[INF_CURR+n-1:2*INF_CURR]
